function energy_value(model, A, M, env, params::iPEPSOptimize{:fermion, :square})
    ...
    for p in 1:N
        ...
        ir = Ni + 1 - i
        jr = mod1(j + 1, Nj)
        e = sum(oc_H_fermion(A[i,j],SdD,Adag[i,j],SdD,SDD,SDD,Adag[i,jr],SdD,SdD,SDD,A[i,jr],SDD,h[1],h[2],
        ...
        ir  = mod1(i + 1, Ni)
        irr = mod1(Ni - i, Ni) 
        e = sum(oc_V_fermion(A[i,j],Adag[i,j],SDD,SdD,SdD,SDD,A[ir,j],Adag[ir,j],SDD,SdD,SdD,SDD,h[1],h[2],
    ...
    params.verbosity >= 3 && println("energy = $(etol/N)")
    return etol/N
end
